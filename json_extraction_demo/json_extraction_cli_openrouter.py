...
# Suppress the "Event loop is closed" error on Windows during cleanup
# This error occurs in asyncio's proactor transport __del__ and is harmless
import atexit

def _suppress_event_loop_error():
    """Suppress asyncio event loop errors during Python shutdown."""
    ...
    asyncio.proactor_events._ProactorBasePipeTransport.__del__ = patched_del

_suppress_event_loop_error()
...
async def extract_peripheral_data(model: str, text: str, prompt_text: str, api_key: str):
    """
    Extracts peripheral data from markdown documentation using OpenRouter API.
    """
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"EXTRACTION SUCCESSFUL: {model}")
        print(f"{'='*80}\n")
        ...
        print(f"\n{'='*80}")
        print(f"ERROR during extraction with {model}")
        print(f"{'='*80}")
        print(f"Error: {e}")
        return None
...
def get_output_filename(input_file: str, model: str) -> str:
    """
    Generates a descriptive output filename based on the input file and model.
    """
    input_stem = Path(input_file).stem
    safe_model = model.replace(":", "_").replace("/", "_")
    return f"{input_stem}_extracted_{safe_model}.json"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract hardware peripheral data from markdown documentation using OpenRouter.",
        epilog="Example: python json_extraction_cli_openrouter.py --input my_doc.md --model minimax/minimax-m2:free"
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        ...
        help="The model to use via OpenRouter (default: minimax/minimax-m2:free)."
    )
    parser.add_argument(
        "--api-key",
    ...
    if not api_key:
        print("Error: OpenRouter API key not provided. Set OPENROUTER_API_KEY environment variable or use --api-key")
        return
    ...
    # --- Run Extraction ---
    result = await extract_peripheral_data(args.model, text, prompt_text, api_key)

    # --- Save Output ---
    if result:
        output_file = args.output if args.output else get_output_filename(args.input, args.model)
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(result.model_dump(), f, indent=2)
        print(f"[OK] Saved extracted data to {output_file}")
