...
"""
A consolidated CLI for extracting hardware peripheral information from markdown documentation using Gemini API.
"""
...
async def extract_peripheral_data(api_key: str, mode: instructor.Mode, text: str, prompt_text: str):
    """
    Extracts peripheral data from markdown documentation using Gemini 2.5 Pro API.
    """
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"EXTRACTION SUCCESSFUL: Gemini 2.5 Pro ({mode.value})")
        print(f"{'='*80}\n")
        ...
        print(f"\n{'='*80}")
        print(f"ERROR during extraction with Gemini 2.5 Pro ({mode.value})")
        print(f"{'='*80}")
        print(f"Error: {e}")
        return None
...
def get_output_filename(input_file: str) -> str:
    """
    Generates a descriptive output filename based on the input file.
    """
    input_stem = Path(input_file).stem
    return f"{input_stem}_extracted_gemini25pro.json"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract hardware peripheral data from markdown documentation using Gemini 2.5 Pro.",
        epilog="Example: python json_extraction_cli_gemini.py --input my_doc.md"
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    if not api_key:
        print("Error: Gemini API key not provided. Set GEMINI_API_KEY environment variable or use --api-key argument.")
        return
    ...
    print(f"Provider: Gemini 2.5 Pro")
    ...
    result = await extract_peripheral_data(api_key, mode, text, prompt_text)
